...
export function createChatwootSessionsAdapter(): SessionsStateAdapter {
  ...
  return {
    ...
    getSessions: (agentBotId) =>
      selectSessionsByAgentBot(getStore().getState(), agentBotId),

    getActiveSessionId: () =>
      selectActiveSessionId(getStore().getState()),

    setActiveSessionId: (id) =>
      getStore().dispatch(setActiveSession({ sessionId: id })),

    getIsLoadingSessions: () =>
      selectIsLoadingSessions(getStore().getState()),

    getIsLoadingMessages: () =>
      selectIsLoadingMessages(getStore().getState()),

    getMessagesBySession: (sessionId) =>
      selectMessagesBySession(getStore().getState(), sessionId),
  };
}
