...
import type { FlashListRef } from '@/hooks/ai-assistant/useAIChatScroll';
...
export const AIChatMessagesList: React.FC<AIChatMessagesListProps> = React.memo(
  ({
    ...
    const renderItem = useCallback(
      ({ item }: { item: UIMessage }) => {
        try {
          ...
          console.error('[AIChatMessagesList] Error rendering message item:', error, item);
          return (
            <View style={themedTailwind.style('p-4 mb-3')}>
              <Text style={themedTailwind.style('text-red-500 text-sm')}>
      ...
      [isLoading, themedTailwind, thoughtsText],
    );
    ...
    return (
      <View style={themedTailwind.style('flex-1 min-h-0')}>
        {isLoadingMessages && activeSessionId ? (
          ...
          <AnimatedFlashListAny
            ...
            contentContainerStyle={themedTailwind.style('p-4')}
        ...
        {/* Tool indicators */}
        {toolCalls.length > 0 && (
          <View style={themedTailwind.style('px-4 pb-2')}>
