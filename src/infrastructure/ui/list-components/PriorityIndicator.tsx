...
/**
 * Fluent-style priority icons matching web PriorityMark.vue.
 * Web uses `priority-{level}` fluent icons rendered in a 14×14 viewBox.
 * These are exclamation-mark/dot patterns (!, !!, !!!) — NOT bar-chart icons.
 *
 * SVG paths sourced from dashboard-icons.json in chatwoot web.
 */
const PriorityUrgentIcon = ({ color }: { color: string }) => (
...
const PriorityHighIcon = ({ color }: { color: string }) => (
...
const PriorityMediumIcon = ({ color }: { color: string }) => (
  <Svg width={14} height={14} viewBox="0 0 14 14" fill="none">
    <Path
...
/**
 * PriorityIndicator — matches web PriorityMark.vue
 *
 * Web spec:
 * - Container: w-3.5 h-3.5 (14px), rounded-sm, inline-flex centered
 * - Urgent: bg-ruby-4, icon color ruby-10
 * - Non-urgent (high/medium/low): bg-slate-4, icon color slate-11
 * - Fluent exclamation-mark style icons (!, !!, !!!)
 */
export const PriorityIndicator = (props: PriorityIndicatorProps) => {
  const themedTailwind = useThemedStyles();
  const isUrgent = props.priority === 'urgent';

  const containerStyle = themedTailwind.style(
    'w-3.5 h-3.5 rounded-sm items-center justify-center',
    isUrgent ? 'bg-ruby-4' : 'bg-slate-4',
  );

  const iconColor = (
    isUrgent ? themedTailwind.style('text-ruby-10') : themedTailwind.style('text-slate-11')
  ).color as string;

  const renderIcon = () => {
    switch (props.priority) {
      case 'urgent':
        return <PriorityUrgentIcon color={iconColor} />;
      case 'high':
        return <PriorityHighIcon color={iconColor} />;
      case 'medium':
        return <PriorityMediumIcon color={iconColor} />;
      case 'low':
        return <PriorityLowIcon color={iconColor} />;
      default:
        return null;
    }
  };

  return <NativeView style={containerStyle}>{renderIcon()}</NativeView>;
};
