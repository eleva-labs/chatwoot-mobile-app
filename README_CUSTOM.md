...
> **Note:** EAS CLI is included as a project dependency and doesn't need to be installed globally. It will be available after running `pnpm install`.
...
### 4. Firebase Setup (Automatic)

Firebase credentials are **automatically set up** when you run `./scripts/pull-env.sh`. The script creates placeholder credential files that allow you to build and run the app locally.

> **Note:** Placeholder credentials allow the app to build and run, but **push notifications will not work**. This is fine for most development work.

#### Enabling Push Notifications (Optional)

If you need push notifications to work, replace the placeholder files with real Firebase credentials:

**Android Configuration:**
1. Go to [Firebase Console](https://console.firebase.google.com)
2. Create/select your Firebase project
3. Add an Android app with package name: `com.chatscommerce.app` (prod) or `com.chatscommerce.app.dev` (dev)
...
**iOS Configuration:**
1. In Firebase Console, add an iOS app with bundle ID: `com.chatscommerce.app` (prod) or `com.chatscommerce.app.dev` (dev)
2. Download `GoogleService-Info.plist` and place in:
...
> **For EAS Cloud Builds:** Firebase credentials are automatically injected from EAS Secrets. No manual setup is required for CI/CD builds.
